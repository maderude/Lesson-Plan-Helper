...
## YOUR TASK
Rewrite ONLY the sections of the lesson plan that caused the failure.
Leave all other sections EXACTLY as they are — same wording, same
structure, same time allocations (unless pacing is a flagged issue).
...
3. **Maintain standard alignment.** Any rewritten section must use
   the EXACT cognitive verb and skill from the confirmed standard.
4. **Keep time allocations realistic.** If you change a section's
...
5. **Enforce Formatting Constraints.** If you rewrite the **Assessment**
...
6. **Do NOT add commentary.** Return ONLY the complete revised
